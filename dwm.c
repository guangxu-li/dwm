...
focusdir(const Arg *arg)
{
	...
	for (c = next; c != s; c = next) {
		...
		if (!ISVISIBLE(c, selmon) || c->isfloating != isfloating) // || HIDDEN(c)
			continue;
...
placedir(const Arg *arg)
{
	...
	for (c = next; c != s; c = next) {
		...
		if (!ISVISIBLE(c, selmon)) // || HIDDEN(c)
			continue;
	...
	if (f && f != s) {
		for (fprior = f->mon->cl->clients; fprior && fprior->next != f; fprior = fprior->next);
		for (sprior = s->mon->cl->clients; sprior && sprior->next != s; sprior = sprior->next);
